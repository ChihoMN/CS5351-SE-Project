...
const sortTasksHelper = (tasks: readonly any[], mode: "manual" | "auto") => {
  if (mode === "manual") return tasks;

  return [...tasks].sort((a, b) => {
    // 1. 第一轮：比拼优先级
    const pA = priorityScore[(a.priority || "medium").toLowerCase()] ?? 2;
    const pB = priorityScore[(b.priority || "medium").toLowerCase()] ?? 2;

    // 如果优先级不同，直接返回结果
    if (pA !== pB) return pB - pA;

    // 2. 第二轮：比拼截止时间 (优先级相同时)
    // 注意：此时 a.dueDate 和 b.dueDate 应该是秒数
    const valA = a.dueDate ? Number(a.dueDate) : Infinity;
    const valB = b.dueDate ? Number(b.dueDate) : Infinity;

    // 🚨 核心修复：处理两个都是 Infinity 的情况
...
export function Column({column, index}: ColumnProps) {
  ...
  const displayedTasks = useMemo(() => {
    return sortTasksHelper(column.tasks, sortMode);
  }, [column.tasks, sortMode]);
          ...
          >
            ...
            >
                ...
                <RadixTooltip.Root open={isFocused} delayDuration={1000}>
                  <RadixTooltip.Trigger asChild>
                    <div className="flex items-center justify-between shrink-0 pt-1">
                      ...
                      {/*/!* 👇👇👇 【新增】排序切换按钮 👇👇👇 *!/*/}
                      {/*<button*/}
                      {/*    onClick={() => setSortMode(prev => prev === "manual" ? "auto" : "manual")}*/}
                      {/*    className={cn(*/}
                      {/*        "w-8 grid place-content-center h-8 shrink-0 rounded-lg ml-1 transition-colors",*/}
                      {/*        sortMode === "auto"*/}
                      {/*            ? "bg-blue-100 text-blue-600 hover:bg-blue-200"*/}
                      {/*            : "text-muted-foreground hover:text-foreground hover:bg-grayA-4"*/}
                      {/*    )}*/}
                      {/*    title={sortMode === "auto" ? "Auto Sort On" : "Manual Sort"}*/}
                      {/*    type="button"*/}
                      {/*>*/}
                      ...
                      <button
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setSortMode(prev => prev === "manual" ? "auto" : "manual");
                          }}
                          // 2. 关键：阻止鼠标按下时的拖拽识别
                          onMouseDown={(e) => {
                            e.stopPropagation();
                          ...
                          className={cn(
                              "w-8 grid place-content-center h-8 shrink-0 rounded-lg ml-1 transition-colors cursor-pointer", // 加了 cursor-pointer
                              sortMode === "auto"
                                  ? "bg-blue-100 text-blue-600 hover:bg-blue-200"
                                  : "text-muted-foreground hover:text-foreground hover:bg-grayA-4"
                          )}
              ...
              <Tasks
                ...
                // readonly={isMember}
                // ref={tasksRef}
                  // 👇👇👇 【修改】这里把 column.tasks 改成 displayedTasks
                  tasks={displayedTasks}
                  columnId={column.id}
                  readonly={isMember}
                  ref={tasksRef}

                  // 👇👇👇 【建议】虽然不知道 Tasks 组件是否支持，但建议加上这个属性
                  // 如果 Tasks 组件不支持这个属性，也不会报错，只是没效果
                  isDragDisabled={sortMode === "auto"}
              />
            </ColumnWrapper>
          </FocusScope>
        );
